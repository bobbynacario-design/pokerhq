...
function getReadinessBankrollFitChoice(buyin) {
  ...
  var rule = bankroll && bankroll.rule ? bankroll.rule : 5;
...
function renderVenueScoutingBlock(venue) {
  ...
  if (!entries.length) {
    return '<div class="scouting-block"><div class="scouting-head"><div><div class="quick-panel-title">Venue scouting</div><div class="scouting-sub">No saved opponent notes for ' + esc(venue) + ' yet.</div></div></div></div>';
  }
  var cards = entries.slice(0, 4).map(function(opp) {
    var tagHTML = (opp.tags || []).map(function(tag) {
      return '<span class="opp-tag ' + esc(tag) + '">' + esc(tag) + '</span>';
    }).join('');
    return '<div class="scouting-entry"><div class="scouting-entry-top"><div class="scouting-name">' + esc(opp.name || 'Unknown opponent') + '</div><div class="scouting-added">' + esc(opp.added || '') + '</div></div>' + (tagHTML ? '<div class="opp-tags scouting-tags">' + tagHTML + '</div>' : '') + '<div class="scouting-note">' + esc(summarizeVillainNotes(opp.notes)) + '</div></div>';
  }).join('');
  var extra = entries.length > 4 ? '<div class="scouting-overflow">+' + (entries.length - 4) + ' more saved for this venue in Opponents.</div>' : '';
  return '<div class="scouting-block"><div class="scouting-head"><div><div class="quick-panel-title">Venue scouting</div><div class="scouting-sub">' + entries.length + ' known opponent' + (entries.length !== 1 ? 's' : '') + ' tagged for ' + esc(venue) + '.</div></div><button class="sec-action" onclick="switchGroup(\'improve\',\'opponents\')">OPEN OPPONENTS</button></div><div class="scouting-grid">' + cards + '</div>' + extra + '</div>';
}

function truncatePrepText(text, max) {
  var t = String(text || '').trim();
  return t.length <= max ? t : t.substring(0, max - 1).trim() + '…';
}

function renderPreSessionPrepBlock() {
  var lessonHands = (window.hands || []).filter(function(h) { return (h.lesson || '').trim(); }).slice(0, 3);
  var note = (window.strategies || []).length ? window.strategies[0] : null;
  if (!lessonHands.length && !note) return '';
  var cards = lessonHands.map(function(h) {
    return '<div class="scouting-entry"><div class="scouting-entry-top"><div class="scouting-name">💡 ' + esc(truncatePrepText(h.lesson, 110)) + '</div></div><div class="scouting-note">' + esc(h.title || 'Hand') + (h.session ? ' · ' + esc(h.session) : '') + '</div></div>';
  }).join('');
  if (note) {
    cards += '<div class="scouting-entry"><div class="scouting-entry-top"><div class="scouting-name">♥ ' + esc(note.topic || 'Study note') + '</div><div class="scouting-added">' + esc(note.week || '') + '</div></div><div class="scouting-note">' + esc(truncatePrepText(note.note, 140)) + '</div></div>';
  }
  return '<div class="scouting-block"><div class="scouting-head"><div><div class="quick-panel-title">Before you sit</div><div class="scouting-sub">Your latest lessons — lock one in before the first hand.</div></div><button class="sec-action" onclick="switchGroup(\'play\',\'hands\')">OPEN HANDS</button></div><div class="scouting-grid">' + cards + '</div></div>';
}
