...
export async function fbSave(key, data) {
  try {
    ...
    await setDoc(doc(db, getFirestorePath(), key), {
...
export async function fbLoadAll() {
  try {
    ...
    for (const key of FIRESTORE_KEYS) {
      const snap = await getDoc(doc(db, getFirestorePath(), key));
...
function startRealtimeListeners() {
  ...
  FIRESTORE_KEYS.forEach(function(key) {
    const unsubscribe = onSnapshot(doc(db, getFirestorePath(), key), function(snap) {
